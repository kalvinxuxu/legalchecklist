...
import { LayoutDashboard, FileText, Upload, LogOut, Settings, ChevronRight, BookOpen, User } from 'lucide-vue-next'
...
const navItems = [
  ...
  { key: 'profile', label: '账户信息', icon: User, to: '/workspace/profile' },
]
    ...
    <aside class="w-60 flex-shrink-0 bg-white border-r border-ivory-200 flex flex-col h-full">
      ...
      <div class="h-16 flex items-center px-5 border-b border-ivory-200">
        <RouterLink to="/" class="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
          ...
          <span class="text-base font-semibold text-navy-800 tracking-tight">法务AI Box</span>
        </RouterLink>
      </div>

      <!-- User info -->
      <div class="px-4 py-4 border-b border-ivory-200">
        <RouterLink to="/workspace/profile" class="flex items-center gap-3 p-3 bg-ivory-50 rounded-xl hover:bg-ivory-100 transition-colors cursor-pointer">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-navy-800 truncate">{{ authStore.displayName }}</p>
